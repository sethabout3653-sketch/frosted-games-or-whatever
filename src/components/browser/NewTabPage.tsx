...
export function NewTabPage({ engine, onEngineChange, onNavigate, onOpenGames }: Props) {
  ...
  const engineName =
    SEARCH_ENGINES.find((e) => e.url === engine)?.name ?? SEARCH_ENGINES[0].name;

  return (
    <div className="frosted-grid relative h-full overflow-y-auto bg-background">
      <div className="flex min-h-full flex-col items-center justify-center px-6 py-16">
        <h1 className="select-none text-7xl font-light tracking-tight text-foreground md:text-8xl">
          Frosted
        </h1>
...
function ShortcutButton({
  ...
  return (
    <button onClick={onClick} className="flex w-20 flex-col items-center gap-3">
      <span className="flex h-14 w-14 items-center justify-center rounded-full border border-border bg-card text-foreground transition-colors hover:bg-accent">
        {children}
      </span>
