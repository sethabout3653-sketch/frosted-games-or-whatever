...
export const CLOAKS: Cloak[] = [
  { id: "none", label: "None", title: "Frosted", icon: "/favicon.ico" },
  {
    id: "classroom",
...
export function resolveCloak(settings: Settings): { title: string; icon: string } | null {
  if (settings.cloakId === "none") return null;
  if (settings.cloakId === "custom") {
    if (!settings.customTitle && !settings.customIcon) return null;
    return { title: settings.customTitle || "Frosted", icon: settings.customIcon };
  }
